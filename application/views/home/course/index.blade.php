...
@if (isset($search_result) && empty($courses->results))
	{{ HTML::alert('info', __('app.course_search_nothing_found')) }}
@else
	{{ $courses->links() }}
	<table class="table table-striped table-hover table-responsive">
		<thead>
			<tr>
				...
				<th>{{ __('app.course_department') }}</th>
		...
		<tbody>
			@foreach ($courses->results as $course)
				<tr>
					...
					<td data-title="{{ __('app.course_department') }}"><abbr title="{{ e($course->department_title_en) }}">{{ e($course->initial) }}</abbr></td>
