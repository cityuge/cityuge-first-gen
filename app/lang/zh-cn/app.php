<?php

return array(

    'appTitle' => 'CityU GE 指南',

    // Common
    'close' => '关闭',

    // Menu
    'nav_skipNav' => '跳至网页内容',
    'nav_toggleNav' => '开启 / 折迭选单',
    'nav_course' => '课程',
    'nav_allCourse' => '所有课程',
    'nav_cityUGECategory' => '精进教育 (GE)',
    'nav_otherCategory' => '其他',
    'nav_advancedSearch' => '进阶搜寻',
    'nav_comment' => '留言',
    'nav_department' => '学术部门',
    'nav_stat' => '统计',

    'nav_search' => '搜寻',
    'nav_searchPlaceholder' => '课程编号或名称',

    'nav_acp' => '管理控制台',
    'nav_logout' => '注销',

    // Admin menu
    'admin_nav_site' => '网站主页',

    // Stats
    'stat_title' => '统计',
    'stat_metaDesc' => '城大精进教育课程 (GE) 统计及排名资料。',
    'stat_hotCourse' => '热门课程',
    'stat_hotCourseNote' => '最多留言的课程。',
    'stat_goodGradeCourse' => '好 Grade 课程',
    'stat_goodGradeCourseNote' => '平均成绩高于或等于 B+ 的首 :limit 个课程。',
    'stat_badGradeCourse' => '烂 Grade 课程',
    'stat_badGradeCourseNote' => '平均成绩低于 B- 的首 :limit 个课程。',
    'stat_lightWorkloadCourse' => '轻松之选',
    'stat_lightWorkloadCourseNote' => '平均工作量低于或等于 2.7 的首 :limit 个课程。',
    'stat_heavyWorkloadCourse' => '繁重之选',
    'stat_heavyWorkloadCourseNote' => '平均工作量高于或等于 3.3 的首 :limit 个课程。',
    'stat_emptyList' => '目前未有资料。',
    'stat_courseCode' => '编号',
    'stat_courseTitle' => '名称',
    'stat_totalComment' => '留言',
    'stat_bayesianGradePoint' => '平均积点',
    'stat_bayesianWorkload' => '平均工作量',
    'stat_analysisReportTitle' => '分析报告',
    'stat_analysisReportDesc' => '<p>本页所提供的课程排名是以简单计算实时生成，只能作基本参考之用。在报读各精进教育 (GE) 课程前，除参考本页面排名外，亦应细阅各课程的相关留言及资料再作判断。</p><p>如要取得详尽的统计资料及分析，请参阅《精进教育课程分析报告》（仅提供英文版）。</p>',
    'stat_analysisReportRead' => '阅读 <small>(ISSUU)</small> <i class="fa fa-angle-right"></i>',
    'stat_analysisReportDownload' => '下载 <small>(Google Drive)</small> <i class="fa fa-angle-right"></i>',

    // Course
    'course' => '{1} 所有课程|[2,Inf] 所有课程：第 :page 页',
    'course_detail_title' => '{1} :courseCode 课程数据|[2,Inf] :courseCode 课程数据：第 :page 页',
    'course_code' => '编号',
    'course_title' => '名称',
    'course_department' => '学系',
    'course_department_course' => '其他课程',
    'course_department_website' => '学系网站',
    'course_category' => '分类',
    'course_level' => '程度',
    'course_offering' => '开办学期',
    'course_offering_none' => '本课程从未开办',
    'course_edgeInfo' => 'EDGE 课程目录',
    'course_form2b' => '表格 2B',
    'course_comment' => '{0} 留言|[1,Inf] :count 则留言',
    'course_avgWorkload' => '平均工作量',
    'course_commentCount' => '留言',
    'course_noComment' => '现时未有留言。',
    'course_metaDesc' => '城大精进教育 (Gateway Education) 课程清单。',
    'course_categoryTitle' => '{1} :category|[2,Inf] :category：第 :page 页',
    'course_categorySemesterTitle' => '{1} :category (:semester)|[2,Inf] :category (:semester)：第 :page 页',
    'course_category_metaDesc' => '城大精进教育 (Gateway Education) :category 课程清单。',
    'course_semester' => '学期',
    'course_semesterNone' => '所有学期',
    'course_detail_metaDesc' => ':courseCode (:courseTitle) 的课程资料、留言、工作量及成绩统计。',
    'course_detail_info' => '基本数据',
    'course_detail_stats' => '统计',
    'course_detail_comment' => '留言',
    'course_assess' => '评核项目',
    'course_assess_exam' => '考试',
    'course_assess_quiz' => '小测',
    'course_assess_report' => '个人报告',
    'course_assess_project' => '小组研习',
    'course_assess_include' => '设有<strong>:task</strong>',
    'course_assess_exclude' => '不设<strong>:task</strong>',
    'course_assess_explain' => '释义',
    'course_assess_model_title' => '评核项目释义',
    'course_assess_def_exam' => '该课程列载于考试时间表 (Master Examination Schedule) 内或在课程资料 (Form 2B) 中有明确表示设有考试。但不包括设于教学周次范围以内的小测及期中测验。',
    'course_assess_def_quiz' => '可以是以个人、小组或混合形式进行。设题方式可以是多项选择题、问答题等。此亦包括期中测验。',
    'course_assess_def_report' => '此包括各种个人文书课业如报告、论文、文章写作及学习历程记录 (Journal)。但不包括小组报告、网上讨论及文章阅读等简短文书课业。',
    'course_assess_def_project' => '此包括小组课业如口头汇报、小组报告、海报或单张设计等。但不包括小组讨论、角色扮演 (Role-play)、组员互评 (Peer-review) 及课堂游戏。',

    // Search
    'course_quickAccessCurrentSemester' => '快速查阅于 :semester 开办的课程',
    'course_search_title' => '搜寻课程',
    'course_search_metaDesc' => '搜寻城大精进教育课程 (GE) 资料。',
    'course_search_empty' => '请输入关键词搜寻。',
    'course_search_keyword' => '课程编号 / 名称',
    'course_search_misc' => '分类、学系及学期',
    'course_search_category' => '分类',
    'course_search_department' => '开办学系',
    'course_search_semester' => '开办学期',
    'course_search_assess' => '评核项目',
    'course_search_assess_yes' => '有',
    'course_search_assess_no' => '无',
    'course_search_assess_dontCare' => '不介意',
    'course_search_nothingFound' => '找不到条件匹配的课程，请改用其他关键词或条件重新搜寻。',
    'course_search_wrongParam' => '网址参数错误，请重新搜寻。',
    'course_search_resultTitle' => '{1} 搜寻结果|[2,Inf] 搜寻结果：第 :page 页',

    // Comment
    'comment' => '{1} 所有留言|[2,Inf] 所有留言：第 :page 页',
    'comment_desc' => '<p>现时共有 :count 则留言。</p>',
    'comment_new' => '撰写留言',
    'comment_newTitle' => '发表对 :courseCode 的留言',
    'comment_warning' => '<h4>留言时请注意</h4>切勿发表失实或人身攻击之留言。一经发现，留言将会被删除而不作任何通知。',
    'comment_id' => '留言编号',
    'comment_ipAdress' => 'IP 地址',
    'comment_semester' => '学期',
    'comment_instructor' => '导师',
    'comment_grade' => '成绩',
    'comment_grade_tooltip' => '成绩：:grade',
    'comment_workload' => '工作量',
    'comment_body' => '留言',
    'comment_recaptcha' => 'reCAPTCHA',
    'comment_createdAt' => '发表日期',
    'comment_submit' => '发表',
    'comment_created' => '留言已发表，感谢你的支持！',
    'comment_metaDesc' => 'CityU GE 指南的最新留言。',
    'comment_permalink' => '永久连结',
    'comment_share' => '分享这则留言',
    'comment_courseDetail' => '更多资料',
    'comment_show_title' => '留言 #:id (:courseCode - :courseTitle)',
    'comment_show_admin' => '管理专用',
    'comment_whois' => 'WHOIS',
    'comment_show_shareDesc' => '<p>你可以用分享按钮 <i class="fa fa-share"></i> 将这则留言分享到各大社交平台，亦可以将本页的网址直接分享给朋友。</p>',

    // Grade
    'grade_p' => 'Pass',
    'grade_f' => 'Fail',
    'grade_x' => 'Dropped',

    // Workload
    'workload_1' => '非常轻松',
    'workload_2' => '轻松',
    'workload_3' => '一般',
    'workload_4' => '繁重',
    'workload_5' => '非常繁重',

    // Department
    'department' => '学术部门',
    'department_noCourse' => '该学术部门并没有提供任何精进教育 (GE) 课程。',
    'department_metaDesc' => '城大的学术部门清单。',
    'department_courseMetaDesc' => '城大:department所提供的精进教育 (GE) 课程清单。',

    // Category
    'category_area1' => 'Area 1',
    'category_area2' => 'Area 2',
    'category_area3' => 'Area 3',
    'category_unireq' => '大学必修',
    'category_e' => '补底班',
    'category_misc' => '其他',
    'category_desc_area1' => '艺术和人文科学',
    'category_desc_area2' => '社会研究，社会和商业组织',
    'category_desc_area3' => '科学和技术',

    // Users & login
    'user_username' => '账户名称',
    'user_password' => '密码',
    'login_rememberMe' => '保持登入',
    'login_title' => '管理控制台',
    'login_submit' => '登入',
    'login_authorizedOnly' => '本页仅供获受权人士进入，请先登入！',
    'login_successful' => '登入成功！',
    'login_unsuccessful' => '你的帐户名称 / 密码组合错误。',

    // Admin
    'admin_title' => 'CityU GE 指南管理控制面板',
    'cache_purge' => '清除所有快取',
    'cache_purged' => '已清除系统内的所有快取！',

    // About
    'about' => '关于本网站',
    'meta_aboutDesc' => 'CityU GE 指南的宗旨、介绍及联络方法。',

    // Footer
    'footer_disclaimer' => '本网站是以实时上载留言的方式运作，所有数据仅供参考，并不构成任何推荐、保证或协议。Swiftzer 对所有留言的真实性、完整性及立场等，不负任何法律责任。<br />本网站与香港城市大学 (City University of Hong Kong, CityU) 并无任何关连。',
    'footer_nav_about' => '关于',
    'footer_nav_rss' => 'RSS',
    'footer_nav_facebookFanPage' => 'Facebook 专页',
    'footer_nav_devBlog' => '开发日志',
    'footer_nav_acp' => '管理控制台',
    'footer_nav_memory' => 'Memory: :memory MB',

    // Social media network
    'socialMedia_facebook' => 'Facebook',
    'socialMedia_twitter' => 'Twitter',
    'socialMedia_googlePlus' => 'Google+',
    'socialMedia_renren' => '人人网',
    'socialMedia_sinaWeibo' => '新浪微博',

    // RSS Feed
    'feed_title' => 'CityU GE 指南',
    'feed_description' => '最新留言',
    'feed_metaTitle' => '订阅 CityU GE 指南的最新留言',
    'feed_course_title' => 'CityU GE 指南：:courseCode',
    'feed_course_description' => ':courseCode 的最新留言',
    'feed_course_metaTitle' => '订阅 :courseCode 的最新留言',

    // Meta
    'meta_homeDesc' => '专为香港城市大学学生而设的精进教育 (Gateway Education) 课程留言平台。分析各课程的成绩及工作量，为选科作好准备。',
    'meta_globalKeyword' => 'GE, Gateway Education, 通识, general education, 精进教育, 好 grade, 烂 grade, TLQ, E 仔班, 补底班, City University of Hong Kong, 香港城市大学, 城市大学, 城大, 又一城市大学, 神大, CityU, City U, Shitty U, ShittyU, AIMS, Hong Kong, 香港',
    'excerptEllipse' => '[...]',

    // Error page
    'error_maintenance' => '维修进行中',
    'error_404' => '找不到网页',

);