	...
	var comment = {
		...
		socialMediaLink: function() {
			...
			$('ul[data-share-list] a').click(function(e) {
				e.preventDefault();
				$el = $(this);
				console.log(that.socialMediaPopUpSpec($el.attr('data-share')));
		...
		socialMediaPopUpSpec: function(mediaKey) {
			...
			return 'scrollbars=yes,resizable=yes,toolbar=no,location=yes,width=' + socialMedia.width + ',height=' + socialMedia.height + ',left=' + ((screen.width - socialMedia.width) / 2) + ',top=' + ((screen.height - socialMedia.height) / 2);
		}
	};
